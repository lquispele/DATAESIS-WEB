...
export default function ExtraoficialesPage() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/api/miembros')
      .then(res => res.json())
      .then(data => {
        const filtrados = Array.isArray(data) ? data.filter(m => m?.tipo === 'extraoficial') : []
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="bg-orange-400 py-12">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-5xl font-bold text-white tracking-tight">Miembros Extraoficiales</h1>
        </div>
      </section>
